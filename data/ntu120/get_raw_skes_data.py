...
def get_raw_bodies_data(skes_path, ske_name, frames_drop_skes, frames_drop_logger):
    ...
    if int(ske_name[1:4]) >= 18:
        data_path = '/dcs/large/u2034358/ntu120/'
        skes_path = osp.join(data_path,'nturgb+d_skeletons120/')
...
def get_raw_skes_data():
    ...
    np.savetxt(osp.join(data_path, 'ntu120', 'raw_data', 'frames_cnt.txt'), frames_cnt, fmt='%d')
...
if __name__ == '__main__':
    save_path = './'
    data_path = '/dcs/large/u2034358/'

    skes_path = osp.join(data_path,'ntu/nturgb+d_skeletons/')
    stat_path = osp.join(save_path, 'statistics')

    if not osp.exists(osp.join(data_path, 'ntu120','raw_data')):
        os.makedirs(osp.join(data_path, 'ntu120','raw_data'))

    skes_name_file = osp.join(stat_path, 'skes_available_name.txt')
    save_data_pkl = osp.join(data_path, 'ntu120', 'raw_data', 'raw_skes_data.pkl')
    frames_drop_pkl = osp.join(data_path, 'ntu120', 'raw_data', 'frames_drop_skes.pkl')

    frames_drop_logger = logging.getLogger('frames_drop')
    frames_drop_logger.setLevel(logging.INFO)
    frames_drop_logger.addHandler(logging.FileHandler(osp.join(data_path, 'ntu120', 'raw_data', 'frames_drop.log')))
    frames_drop_skes = dict()
