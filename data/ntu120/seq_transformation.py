...
# activate yinka_env for sklearn
from sklearn.model_selection import train_test_split

# the next line is for utils
sys.path.append('/dcs/pg20/u2034358/codes/infogcn')
...
def align_frames(skes_joints, frames_cnt):
    ...
    for idx, ske_joints in enumerate(skes_joints):
        num_frames = ske_joints.shape[0]
        num_bodies = 1 if ske_joints.shape[1] == 75 else 2
        if num_bodies == 1: # I changed the below myself
...
if __name__ == '__main__':
    ...
    for evaluation in evaluations:
        ...
        create_aligned_dataset(file_list=[osp.join(save_path,'NTU120_%s.npz' % evaluation)])
    # create_aligned_dataset(file_list=[osp.join(save_path,'NTU120_CSet.npz'),
    #                                     osp.join(save_path,'NTU120_CSub.npz')])

    print("We are now done transforming...")
