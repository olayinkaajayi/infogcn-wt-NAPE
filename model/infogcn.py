...
class InfoGCN(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, in_channels=3,
                 drop_out=0, num_head=3, noise_ratio=0.1, k=0, gain=1, PE_name=''):
        ...
        # Position Encoding
        if PE_name != '':
            PE = TT_Pos_Encode(hidden_size=base_channel, N=self.num_point, d=8, PE_name=PE_name)
            pos_encode = PE.get_position_encoding(need_plot=False)
            pos_embedding = pos_encode.unsqueeze(0) #shape: 1 x N x base_channel
            self.register_buffer('pos_embedding', pos_embedding)
            print("Using NAPE Position Encoding...")
        else:
    ...
    def forward(self, x):
        ...
        pos_encode = self.pos_embedding[:, :self.num_point].detach() if self.PE_name != '' else self.pos_embedding[:, :self.num_point]
        x += pos_encode
